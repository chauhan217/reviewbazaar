        ...
        <div class="col-md-6">
          <div class="card h-100">
            <div class="blog-img">
                ...
                @endif
               <span class="badge">{{ $blog->created_at->format('F j, Y') }}</span>

            </div>
            <div class="card-body">
                <div class="pb-2"><a class="fs-5 text-decoration-none title" href="{{ url('blog/' . $blog->slug) }}">{{ $blog->title }}</a></div>
                <p class="card-text">
                    {!! \Illuminate\Support\Str::limit(strip_tags($blog->description), 100, '...') !!}
                </p>
               <div class="pt-3"><a class="btn btn-primary btn-sm" href="{{ url('blog/' . $blog->slug) }}">Read more</a></div> 
            </div>
          </div>
        </div>
        @endforeach
