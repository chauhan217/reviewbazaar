    ...
    <div class="row g-3">
      ...
      <div class="col-md-4">
        <div class="card h-100">
          ...
          @endif
          <span class="badge">{{ $blog->created_at->format('M d, Y') }}</span>
          </div>
          <div class="card-body">
            <div class="pb-2"><a class="fs-5 text-decoration-none title lh-1" href="{{ url('blog/' . $blog->slug) }}">{{ $blog->title }}</a></div>
            <p class="card-text">{!! \Illuminate\Support\Str::limit(strip_tags($blog->description), 100, '...') !!}</p>
            <div class="pt-3"><a class="small btn btn-primary btn-sm" href="{{ url('blog/' . $blog->slug) }}">Read more</a></div>
          </div>
        </div>
      </div>
     @endforeach
